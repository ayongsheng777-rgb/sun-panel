...
12. 用户说「补全网址 / 网址信息补全 / 把内容补齐 / 缺网址的补上」→ panel.backfill_urls：
    联网搜官网自动补全网址并顺带补图标；搜不到官网的不询问，自动排到分组最后面。
13. 用户指定了分组时，两个补齐工具的 params.group 这样填：
    - 说了全名就填全名（如「AI动画」）；
    - 说「带XX字眼 / 名字包含XX / XX相关的分组」就只填那个字眼（如「AI」），
      系统会匹配所有名字里含该字眼的分组；
    - 没提任何分组就留空，表示扫描全部分组。
14. 用户输入可能有同音字或手误（例如「动面」其实是「面板」），按语义理解即可；
    分组名尽量从「当前面板数据」里取真实名称，不要自己编造分组名。
